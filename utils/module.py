...
from utils.refine_block import RefineBlock, RefineDecoder
from utils.matrix_utils import matrix2flat


class ModelManager(nn.Module):

    def __init__(self, args, num_word, num_slot, num_intent, share_decoder = False):
        ...
        self.__embedding = EmbeddingCollection(self.__num_word,
                                               self.__args.word_embedding_dim)

        # Initialize an LSTM Encoder object.
        self.__encoder = LSTMEncoder(self.__args.word_embedding_dim,
                                     self.__args.encoder_hidden_dim,
                                     self.__args.dropout_rate)

        # Initialize an self-attention layer.
        self.__attention = SelfAttention(self.__args.word_embedding_dim,
        ...
        if self.share_decoder:
            self.__refine_slot_decoder = self.__slot_decoder
            self.__refine_intent_decoder = self.__intent_decoder
        else:
            self.__refine_slot_decoder = LSTMDecoder(
                self.__args.encoder_hidden_dim +
                self.__args.attention_output_dim,
                self.__args.slot_decoder_hidden_dim,
                self.__num_slot,
            ...
            self.__refine_intent_decoder = LSTMDecoder(
                self.__args.encoder_hidden_dim +
                self.__args.attention_output_dim,
                self.__args.intent_decoder_hidden_dim,
                self.__num_intent,
        ...
        self.__intent_embedding = nn.Embedding(self.__num_intent,
                                               self.__num_intent)
        ...
        #     pass
        # else:
        # print("self.__encoder_output_dim", self.__encoder_output_dim)
        # print("self.slot_decoder_hidden_dim", self.__args.slot_decoder_hidden_dim)
        hidden_size = self.__args.attention_output_dim + self.__args.encoder_hidden_dim
        #self.block = RefineBlock(hidden_size, num_intent, num_slot, graph_hidden_dim=128, graph_output_dim = args.encoder_hidden_dim+args.attention_output_dim)
        self.block = RefineDecoder(args.block_num,
                                   hidden_size,
                                   num_intent,
                                   num_slot,
    ...
    def show_summary(self):
        ...
        print('\tnumber of word:                            {};'.format(
            self.__num_word))
        print('\tnumber of slot:                            {};'.format(
            self.__num_slot))
        print('\tnumber of intent:						    {};'.format(self.__num_intent))
        print('\tword embedding dimension:				    {};'.format(
            self.__args.word_embedding_dim))
        print('\tencoder hidden dimension:				    {};'.format(
            self.__args.encoder_hidden_dim))
        print('\tdimension of intent embedding:		    	{};'.format(
            self.__args.intent_embedding_dim))
        print('\tdimension of slot embedding:			    {};'.format(
            self.__args.slot_embedding_dim))
        print('\tdimension of slot decoder hidden:  	    {};'.format(
            self.__args.slot_decoder_hidden_dim))
        print('\tdimension of intent decoder hidden:        {};'.format(
            self.__args.intent_decoder_hidden_dim))
        print('\thidden dimension of self-attention:        {};'.format(
            self.__args.attention_hidden_dim))
        print('\toutput dimension of self-attention:        {};'.format(
            self.__args.attention_output_dim))
    ...
    def forward(self,
                ...
                forced_intent=None,
                self_loop=1,
                force_refine_slot=None,
                force_refine_intent=None,
                topk=3):
        ...
        pred_intent = self.__intent_decoder(hiddens,
                                            seq_lens,
        ...
        pred_slot = self.__slot_decoder(hiddens,
                                        seq_lens,
                                        forced_input=forced_slot,
                                        extra_input=feed_intent)

        refine_pred_intent = pred_intent
        refine_pred_slot = pred_slot
        for _ in range(self_loop):
            hiddens, pos_hidden, refine_pred_intent, refine_pred_slot = self.block(
                hiddens,
                seq_lens,
                refine_pred_intent,
                refine_pred_slot,
                True,
                force_slot=force_refine_slot,
                force_intent=force_refine_intent,
                topk=topk)
        ...
        if n_predicts is None:
            return F.log_softmax(pred_slot, dim=1), \
                    ...
                    F.log_softmax(refine_pred_slot,dim=1),\
                    F.log_softmax(refine_pred_intent, dim=1),\
                    hiddens, pos_hidden

        else:
            _, slot_index = pred_slot.topk(n_predicts, dim=1)
            _, intent_index = pred_intent.topk(n_predicts, dim=1)
            _, refine_slot_index = refine_pred_slot.topk(n_predicts, dim=1)
            _, refine_intent_index = refine_pred_intent.topk(n_predicts, dim=1)

            return slot_index.cpu().data.numpy().tolist(), \
                   intent_index.cpu().data.numpy().tolist(), \
                   refine_slot_index.cpu().data.numpy().tolist() ,\
                   refine_intent_index.cpu().data.numpy().tolist()

    def golden_intent_predict_slot(self,
                                   text,
                                   seq_lens,
                                   golden_intent,
                                   n_predicts=1):
...
class EmbeddingCollection(nn.Module):
    ...
    def __init__(self, input_dim, embedding_dim, max_len=5000):
        ...
        # Word vector encoder.
        self.__embedding_layer = nn.Embedding(self.__input_dim,
...
class QKVAttention(nn.Module):
    ...
    def forward(self, input_query, input_key, input_value):
        ...
        score_tensor = F.softmax(
            torch.matmul(linear_query, linear_key.transpose(-2, -1)) /
...
class SelfAttention(nn.Module):
    ...
    def forward(self, input_x, seq_lens):
        ...
        flat_x = torch.cat(
            [attention_x[i][:seq_lens[i], :] for i in range(0, len(seq_lens))],
            dim=0)
        return flat_x
